...
const Challenge = _ => {
    ...
    raceState.startrace = _ => {
        ...
        if(raceState.ptotal > raceState.ototal){
            console.log(raceState.bet)
            Player.updatebank(raceState.id, {bankAccount: raceState.money + bet})
            .then(_ => {
                Player.updateexperience(raceState.id, {experience: raceState.experience + 5})
                .then(_ => {
                    Player.updatewin(raceState.id, {win: raceState.win + 1})
                    .then(_ =>{
                        setraceState({...raceState, logarr, ototal, ptotal, prerace: false})
                    })
                })
            })
        } else if (raceState.ptotal < raceState.ototal) {
            Player.updateloss(raceState.id, {loss: raceState.loss + 1})
            .then(_ => {
                Player.updatebank(raceState.id, {bankAccount: raceState.money - bet})
                .then(_ => {
                    setraceState({...raceState, logarr, ototal, ptotal, prerace: false})
            ...
            Player.updateexperience(raceState.id, {experience: raceState.experience + 2})
            .then(_ => {
                Player.updatetie(raceState.id, {tie: raceState.tie + 1})
                .then(_ => {
                    setraceState({...raceState, logarr, ototal, ptotal, prerace: false})
                })
            })
